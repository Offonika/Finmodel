    ...
    for r in rows:
        ...
        if key not in agg:
            agg[key] = {
                ...
                'Код_номенклатуры': r.get('nm_id') or "",
...
def import_wb_detailed_reports(wb=None):
    ...
    if old_facts and len(old_facts) > 1:
        ...
        for r in old_facts[1:]:
            if not r:
                continue
            key = (
                str(r[idx_f['Организация']]),
                str(r[idx_f['Номер_отчёта']]),
                str(r[idx_f['Код_номенклатуры']]),
                str(r[idx_f['Артикул_продавца']])
        ...
        for period_start, period_end in periods:
            ...
            while True:
                ...
                for rec in records_batch:
                    key = (
                        rec['Организация'],
                        str(rec['Номер_отчёта']),
                        str(rec['Код_номенклатуры']),
                        str(rec['Артикул_продавца'])
    ...
    hdr = [
        'Организация', 'Дата', 'Предмет', 'Артикул_продавца', 'Код_номенклатуры', 'Название', 'Номер_отчёта',
